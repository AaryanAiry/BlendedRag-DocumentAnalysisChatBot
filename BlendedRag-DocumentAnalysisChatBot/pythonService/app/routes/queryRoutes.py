...
# Implementing a max-score fusion [to be replaced later by RRF or weighted sum]
router = APIRouter()
logger = getLogger(__name__)
embedding_client = EmbeddingClient()  # fixed variable name
...
class QueryResponse(BaseModel):  # fixed missing colon
    docId: str
    query: str
...
def queryEndpoint(req: QueryRequest):
    ...
    for q in refinedQueries:
        results = retrieveTopK(req.docId, q, topK=req.topK)  # ensure this matches retriever signature
        for r in results:
    ...
    fused = sorted(
        [
            {"chunkIndex": i, "text": v["text"], "score": v["score"]}
            for i, v in scoreMap.items()
        ],
